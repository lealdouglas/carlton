...
import pytest
from config_ingestor import ConfigIngestor
...
@patch('config_ingestor.ConfigValidator.validate_args')
@patch('config_ingestor.PathBuilder.build_adls_paths')
...
@patch('config_ingestor.ConfigValidator.validate_args')
@patch('config_ingestor.ConfigIngestor.get_params_path')
...
def test_config_ingest_src_json(
    mock_get_params_path, mock_validate_args, config_json
):
    """
    Testa a função config_ingest_src para arquivos JSON.
    Tests the config_ingest_src function for JSON files.
    """
    mock_get_params_path.return_value = config_json
    result = ConfigIngestor.config_ingest_src(config_json)
    mock_validate_args.assert_any_call(
        ['type_run', 'file_extension', 'file_resource'], config_json
    )
    mock_get_params_path.assert_called_once_with(config_json)
